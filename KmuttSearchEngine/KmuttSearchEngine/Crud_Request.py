...
def request_update(request, operation):
    ...
    try:
        ...
        if request.method == "POST":
            if (request.POST.get('question') and request.POST.get('department_id') and request.POST.get('remark')):
                if operation == 'update':
                    ...
                    if (request.POST.get('question') and request.POST.get('department_id') 
                    and request.POST.get('remark')):
                        saverecord = QArequest()
...
def admin_approve(request, admin_id, id):
    ...
    try:
        current_time = datetime.datetime.now().replace(microsecond=0)
        if request.method == "POST":
            if (request.POST.get('question') and request.POST.get('answer')) and request.POST.get('department_id') and request.POST.get('remark'):
